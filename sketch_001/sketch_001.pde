...
void draw(){
  ...
  int number_of_years = redtitles.length;
...
for (int j = 0; j < all_3.length; j++) {
   String[] subArray = all_3[j];
...
for (int j = 0; j < all_4.length; j++) {
   String[] subArray = all_4[j];
